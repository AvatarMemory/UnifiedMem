"""Adapted from LongMemEval/src/generation/run_generation.py
"""
...
def parse_args():
    ...
    parser.add_argument('--use_raw_session', action='store_true', default=False)    # whether to use raw session text for generation
    parser.add_argument('--merge_key_expansion_into_value', type=str, choices=['merge', 'none', 'replace'], default='none')     # merge key expansion into value
...
def prepare_prompt(entry, retriever_type, topk_context: int, useronly: bool, history_format: str, cot: bool, tokenizer, tokenizer_backend, max_retrieval_length, merge_key_expansion_into_value, con=False, con_client=None, con_model=None):    
    ...
    has_expansion = False
    if merge_key_expansion_into_value != 'none':
        corpusid2retvalue = {}
        expansion_dict = {'session-summ': 'summary', 'session-keyphrase': 'keyphrases', 'session-userfact': 'user_facts',}
        for ret_result_entry in entry['retrieval_results']['ranked_items']:
            if ret_result_entry['corpus_id'] in corpusid2retvalue:
                if ret_result_entry['expansion_type'] != 'original':  # we dont need original session
                    expansion_type = expansion_dict.get(ret_result_entry['expansion_type'], 'Relevant Info')
                    if expansion_type in corpusid2retvalue[ret_result_entry['corpus_id']] and corpusid2retvalue[ret_result_entry['corpus_id']][expansion_type] != ret_result_entry['text']:
                        corpusid2retvalue[ret_result_entry['corpus_id']][expansion_type] += ' ' + ret_result_entry['text']
                    else:
                        corpusid2retvalue[ret_result_entry['corpus_id']][expansion_type] = ret_result_entry['text']
                    has_expansion = True
            else:
    ...
    # get retrieved chunks
        ...
        for ret_result_entry in entry['retrieval_results']['ranked_items']:
            # since we store the expansions as the ranked item, we need to exclude them here to avoid duplication
            if ret_result_entry.get('expansion_type', 'original') != 'original':
                continue

            # handle optional merging key into the value
            if merge_key_expansion_into_value is None or merge_key_expansion_into_value == 'none':
                if useronly:
                    retrieved_chunks.append((corpusid2date[ret_result_entry['corpus_id']], [x for x in corpusid2entry[ret_result_entry['corpus_id']] if x['role'] == 'user']))
                else:
                    ...
                    retrieved_chunks.append((corpusid2date[ret_result_entry['corpus_id']], corpusid2entry[ret_result_entry['corpus_id']]))
            elif merge_key_expansion_into_value == 'replace':
                assert has_expansion, "No available expansion to replace!"
                try:
                    retrieved_chunks.append((corpusid2date[ret_result_entry['corpus_id']], corpusid2retvalue[ret_result_entry['corpus_id']]))
                except KeyError:
                    print('Warning: no expansion found for corpus id {}'.format(ret_result_entry['corpus_id']))
                    retrieved_chunks.append((corpusid2date[ret_result_entry['corpus_id']], corpusid2entry[ret_result_entry['corpus_id']]))
            elif merge_key_expansion_into_value == 'merge':
                assert has_expansion, "No available expansion to merge!"
                if ret_result_entry['corpus_id'] in corpusid2retvalue:  # it's possible that some session doesn't have expansion (due to original longmem missed these)
                    expansions = corpusid2retvalue[ret_result_entry['corpus_id']]
                else:
                    print('Warning: no expansion found for corpus id {}'.format(ret_result_entry['corpus_id']))
                    expansions = {}

                if useronly:
                    retrieved_chunks.append((corpusid2date[ret_result_entry['corpus_id']], expansions, [x for x in corpusid2entry[ret_result_entry['corpus_id']] if x['role'] == 'user']))
                else:
                    retrieved_chunks.append((corpusid2date[ret_result_entry['corpus_id']], expansions, corpusid2entry[ret_result_entry['corpus_id']]))
            else:
                raise NotImplementedError
    ...
    for i, cur_item in enumerate(retrieved_chunks):
        ...
        if history_format == 'json':
            if merge_key_expansion_into_value == 'merge':
                expansions = {expansion_type: chunk_expansion_entry[expansion_type] for expansion_type in chunk_expansion_entry}
                if args.use_raw_session or not expansions:
                    sess_string = '\n' + json.dumps({**expansions, 'original_session': chunk_entry}, indent=2)
                else:
                    sess_string = '\n' + json.dumps(expansions, indent=2)
            else:
                sess_string = '\n' + json.dumps(chunk_entry, indent=2)
...
def main(args):
    ...
    client = OpenAI(
        ...
        base_url=args.openai_base_url if 'gpt' not in args.model_name else None,
    )
