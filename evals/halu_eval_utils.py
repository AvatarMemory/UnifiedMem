...
OPENAI_BASE_URL = cfg.get_stage_base_url('qa_eval', None)
OPENAI_API_KEY = cfg.get_stage_api_key('qa_eval', None)
MODEL = cfg.get_stage_model('qa_eval', 'gpt-4o-mini')
...
openai_temperature = cfg.getenv('OPENAI_TEMPERATURE', None)
if openai_temperature:
    common_params["temperature"] = float(openai_temperature)

openai_timeout = cfg.getenv('OPENAI_TIMEOUT', None)
if openai_timeout:
...
def get_client(stage: str = "qa_eval"):
    client = clients.get(stage)
    if client is None:
        client_cfg = cfg.get_stage_client_config(stage, default_model='gpt-4o-mini')
        if OpenAI is None:
            raise RuntimeError(
                "The `openai` Python package is required for HaluMem evaluation steps that call the judge model. "
...
def llm_request(prompt, stage: str = "qa_eval"):
    """
    ...
    model = cfg.get_stage_model(stage, 'gpt-4o-mini')
    response_obj = get_client(stage).chat.completions.create(
        model=model,
        messages=[
            {
                'role': 'user', 
...
def llm_request_for_json(prompt, stage: str = "qa_eval"):
...
def evaluation_for_memory_integrity(
    ...
    result = llm_request_for_json(prompt, stage="qa_eval")
