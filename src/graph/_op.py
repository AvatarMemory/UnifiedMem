...
async def extract_entities(
    ...
    entity_extract_prompt = global_config["entity_extraction_prompt"]
    context_base = dict(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        time_stamp = chunk_dp["timestamp"]
        hint_prompt = entity_extract_prompt.format(**context_base, input_text=content, dialogue_time=time_stamp)
        final_result = await use_llm_func(hint_prompt, chunk_id=chunk_key)
...
async def _find_most_related_community_from_entities(
    ...
    use_community_reports = sorted_community_datas[:5]
...
def get_ranking_method(query_param: QueryParam):
    for keyword in query_param.graphrag_mode:
        if keyword.startswith("rank-"):
            return keyword
    return "rank-entity-count"
...
async def _find_most_related_text_unit_from_entities(
    ...
    query: str = None,
    remove_user_node: bool = False,
):
    # 0. Get all one-hop nodes and edges (used to COUNT RANKING or one-hot-expand)
    ...
    all_one_hop_nodes_data = [
        {"entity_name": k, **n}
        for k, n in zip(all_one_hop_nodes, all_one_hop_nodes_data)
    ]

    # 1. activate one-hot-expand nodes
    # NOTE: this function don't return the one-hot entities, only return the text units found by one-hop entities (and the initial top-k entities)
    if "one-hot-expand" in query_param.graphrag_mode:
        seen_nodes = set()
        for dp in node_datas:
            seen_nodes.add(dp["entity_name"])
        for dp in all_one_hop_nodes_data:
            if dp["entity_name"] not in seen_nodes:
                seen_nodes.add(dp["entity_name"])
                node_datas.append(dp)
        # NOTE: update new one_hop_nodes (to count relation counts) based on new node_datas
        edges = await knowledge_graph_inst.get_nodes_edges_batch([dp["entity_name"] for dp in node_datas])
        all_one_hop_nodes = set()
        for this_edges in edges:
            if not this_edges:
    ...
    # user node会一下把所有text unit都加进来（且他们的entity order会相同），尝试去掉它
    if remove_user_node or "remove-user-node" in query_param.graphrag_mode:
        node_datas = [dp for dp in node_datas if dp["entity_name"].lower().replace("\"", "") != "user"]
    ...
    for index, (this_text_units, this_edges) in enumerate(zip(text_units, edges)):
        for c_id in this_text_units:
            # TODO: 这里发现通过多个实体可以找到同一个chunk时直接continue了，实际上这个重要信息可以给该chunk加分
    ...
    # 2. Calculate vector similarity if needed
    ranking_method = get_ranking_method(query_param)
    if "vector" in ranking_method and query:
        query_vec = await embedding_func([query])
        query_vec = query_vec[0]
        
        # Batch embed chunks
        chunk_contents = [x["data"]["content"] for x in all_text_units]
        if chunk_contents:
            chunk_vecs = await embedding_func(chunk_contents)
            
            # Calculate cosine similarity
            # query_vec: (dim,)
            # chunk_vecs: (n, dim)
            # sim = (chunk_vecs @ query_vec) / (|chunk_vecs| * |query_vec|)
            # Assuming embedding_func returns numpy array
            
            # Normalize
            query_vec_norm = np.linalg.norm(query_vec)
            chunk_vecs_norm = np.linalg.norm(chunk_vecs, axis=1)
            
            dot_prod = chunk_vecs @ query_vec
            sims = dot_prod / (chunk_vecs_norm * query_vec_norm + 1e-9)
            
            for i, unit in enumerate(all_text_units):
                unit["sim_score"] = sims[i]
        else:
             for unit in all_text_units:
                unit["sim_score"] = 0.0

    # 3. Sorting
    if ranking_method == "rank-vector" and query:
        all_text_units = sorted(
            all_text_units, key=lambda x: -x.get("sim_score", 0.0)
        )
    elif ranking_method == "rank-entity" and query:
        all_text_units = sorted(
            all_text_units, key=lambda x: x["order"]
        )
    elif ranking_method == "rank-count" and query:
        all_text_units = sorted(
             all_text_units, key=lambda x: -x["relation_counts"]
        )
    elif ranking_method == "rank-entity-vector" and query:
        all_text_units = sorted(
             all_text_units, key=lambda x: (x["order"], -x.get("sim_score", 0.0))
        )
    elif ranking_method == "rank-count-vector" and query:
        all_text_units = sorted(
             all_text_units, key=lambda x: (-x["relation_counts"], -x.get("sim_score", 0.0))
        )
    elif ranking_method == "rank-entity-count-vector" and query:
        all_text_units = sorted(
            all_text_units, key=lambda x: (x["order"], -x["relation_counts"], -x.get("sim_score", 0.0))
        )
    else: # default: rank-entity-count
        all_text_units = sorted(
    ...
    # NOTE: we add offline chunk id (same with rayrag), used in the merging process
    # so that all_text_units don't meet the TextChunkSchema type
...
async def _build_query_context(
    ...
    # graphrag_mode: list = ["chunk", "entity", "relation", "community"]
    # 这里rank是node degree，把向量相似度得分扔掉了
    entities_context = []
    if "entity" in query_param.graphrag_mode:
        for i, n in enumerate(node_datas):
            if n.get("description", ""):
                entities_context.append({
                    ...
                    "content": n.get("description", "UNKNOWN"),
                    "rank": n["rank"],
                })

    communities_context = []
    if "community" in query_param.graphrag_mode:
        use_communities = await _find_most_related_community_from_entities(
            node_datas, query_param, community_reports, tokenizer_wrapper
        )
        for i, c in enumerate(use_communities):
            communities_context.append({
                "res_type": "community",
                "content": c["report_string"],
            })
    
    relations_context = []
    if "relation" in query_param.graphrag_mode:
        use_relations = await _find_most_related_edges_from_entities(
            node_datas, query_param, knowledge_graph_inst, tokenizer_wrapper
        )
        for i, e in enumerate(use_relations):
    ...
    # NOTE: 这些都没有给出对应得分，只是根据实体等信息进行了排序然后根据token limit裁剪
    # 在后续与rayrag的融合中我们需要chunk得分来进行加权融合，但是两边得分不是相同含义空间的
    # 目前策略是后续rayrag和graphrag找到的chunk都通过general model与query做相似度匹配，统一使用此得分
    text_units_context = []
    if "chunk" in query_param.graphrag_mode:
        use_text_units = await _find_most_related_text_unit_from_entities(
                node_datas, query_param, text_chunks_db, knowledge_graph_inst, tokenizer_wrapper, embedding_func, query
            )
        for i, t in enumerate(use_text_units):
    ...
    logger.info(
        f"Using {len(entities_context)} entites, {len(communities_context)} communities, {len(relations_context)} relations, {len(text_units_context)} text units"
    )
    final_context = entities_context + relations_context + text_units_context + communities_context

    return final_context


async def _build_query_context_from_relations(
    query,
    knowledge_graph_inst: BaseGraphStorage,
    ...
    # step1: find topic relationships
    results = await relations_vdb.query(query, top_k=query_param.top_k)
    if not len(results):
        return []
    ...
    edge_datas = [
        {**e, "src_id": r["src_id"], "tgt_id": r["tgt_id"], "sim_score": float(r.get("distance", 0.0)), "rank": d}
        for r, e, d in zip(results, edge_datas, edge_degrees)
        if e is not None
    ]

    # step2: find associated entities
    # NOTE：要确保这里得到的entities是按照relation相似度排序的，形式上是个sorted list，后面找chunk时是按这个顺序（entity order）作为first sort key
    # TODO：严格上这里src和tgt应该是一个entity order，为实现方便先直接忽略
    related_entities = []
    for e in edge_datas:
        if e["src_id"] not in related_entities:
            related_entities.append(e["src_id"])
        if e["tgt_id"] not in related_entities:
            related_entities.append(e["tgt_id"])
    
    node_datas = await knowledge_graph_inst.get_nodes_batch(related_entities)
    node_degrees = await knowledge_graph_inst.node_degrees_batch(related_entities)
    
    node_datas = [
        {**n, "entity_name": k, "rank": d}
        for k, n, d in zip(related_entities, node_datas, node_degrees)
        if n is not None
    ]
    ...
    relations_context = []
    if "relation" in query_param.graphrag_mode:
        # Sort relations by similarity score (primary) and weight (secondary)
        #  use_relations = sorted(
        #     edge_datas, key=lambda x: (x["sim_score"], x["weight"]), reverse=True
        #  )
        ...
        for e in edge_datas:
            relations_context.append({
                ...
                "weight": e["weight"],
                "rank": e["rank"],
                "sim_score": e["sim_score"],
                "content": e["description"],
            })

    text_units_context = []
    if "chunk" in query_param.graphrag_mode:
        # Use existing helper with our nodes
        use_text_units = await _find_most_related_text_unit_from_entities(
                node_datas, query_param, text_chunks_db, knowledge_graph_inst, tokenizer_wrapper, embedding_func, query
            )
    ...
    final_context = relations_context + entities_context + text_units_context + communities_context

    return final_context


async def longmemeval_query(
    query,
    knowledge_graph_inst: BaseGraphStorage,
    entities_vdb: BaseVectorStorage,
    relations_vdb: BaseVectorStorage,
