...
class GraphRAG:
    ...
    # text embedding
    # Choose default embedding function based on ENV: EMBEDDING_MODEL
    # Supported values: 'contriever', 'external' (calls EMBEDDING_API_URL), 'bedrock', 'azure', otherwise 'openai'
    # Use backend-controlled dispatcher which will choose between external API,
    # contriever, or openai based on environment (EMBEDDING_API_URL / EMBEDDING_MODEL).
    ...
    entity_extraction_prompt: str = PROMPTS["entity_relation_extraction_short"]

    # storage
    entity_vdb_namespace: str = "entities"
    relation_vdb_namespace: str = "relations"
    ...
    def __post_init__(self):
        ...
        self.entities_vdb = (
            self.vector_db_storage_cls(
                namespace=self.entity_vdb_namespace,
        ...
        self.relations_vdb = (
            self.vector_db_storage_cls(
                namespace=self.relation_vdb_namespace,
                global_config=asdict(self),
                embedding_func=self.embedding_func,
            )
        )
    ...
    def insert(self, doc_id: str, doc_content: str, chunks: List[dict]):
        loop = always_get_an_event_loop()
        return loop.run_until_complete(self.ainsert(doc_id, doc_content, chunks))
    ...
    async def ainsert(self, doc_id: str, doc_content: str, chunks: List[dict]):
        ...
        try:
            ...
            for idx, chunk in enumerate(chunks):
                tokens = self.tokenizer_wrapper.encode(chunk['content'])
                inserting_chunks[chunk['id']] = {
                    "full_doc_id": doc_id,
